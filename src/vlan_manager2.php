...
function vlan_manager2() {
	...
	*/
	...
	while ($db->next_record()) {
		...
		$db2->query("select vlans.*,servers.server_id,server_hostname,switch,port,graph_id from vlans left join switchports on (vlans=vlans_id or vlans like concat('%,',vlans_id,',%') or vlans like concat('%,',vlans_id) or vlans like concat (vlans_id, ',%') ) left join assets on assets.id=asset_id left join servers on order_id=servers.server_id  where vlans_block='{$network_id}' order by {$order};", __LINE__, __FILE__);
